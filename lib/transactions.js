      ...
      return  getAssetRegistry('org.decentralized.energy.network.Energy')
        ...
        .then(function (all_assetRegistry){
            ...
            energy_list.forEach(function(element){
                if (!(element.units in energy_object_dict)){
          					// 记录[出现相同units的次数，第一次出现的时间，最新一次出现的时间]
                    energy_object_dict[element.units] = [1, element.value, element.value];
                }else{
          					// 更新记录[出现相同units的次数 + 1，第一次出现的时间，最新一次出现的时间]
                    energy_object_dict[element.units][0] += 1;
          					energy_object_dict[element.units][2] = element.value;
                }
            });
            ...
            energy_list.forEach(function(element){
                if (ip_list.indexOf(element.units) != -1){  // !!!数组包含的写法，不能用in
                    element.flag = "1";
                }
            });
        })
                ...
                .then(function (){
                      return getAssetRegistry('org.decentralized.energy.network.DDoS')
                          .then(function (ddosAssetResident){
                              ...
                              // ddos.ddosID = 'DDOS_2';
                              ddos.units = ip_list[0]; //'127.0.0.1';
